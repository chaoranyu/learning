...
load_igb_uio_module()
{
    ...
    echo -e ">>>>> [${our_entry}] Insert IGB UIO module <<<<<\n" ##################
}
...
set_non_numa_pages()
{
    ...
    echo -e ">>>>> [${our_entry}] Setup hugepage mappings for non-NUMA systems <<<<<\n" ######################
}
...
bind_devices_to_igb_uio()
{
    ...
    echo -e ">>>>> [${our_entry}] Bind Ethernet/Crypto device to IGB UIO module <<<<<\n" ################
}
