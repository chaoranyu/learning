/*
I use this program in order to encrypt messages inside C programs!!!


Compilation
-----------
How to compile? (assuming this file is named Omer_and_Lior.c)
>>>> gcc -o encryptor.exe Omer_and_Lior.c

Encryption
----------
Now, if I want to encrypt a file name message.txt containing
some message (for example: "secret hello world!"), I run:

>>>> encryptor.exe < message.txt > enc.c

this will result in a C code named enc.c which contains the encrypted
message.

Decryption
----------
In order to decrypt I compile the encrypted file and run it. 
Weird ha??!? The decryption is done using the compiler!
